...
def kpi_card(label, value, goal_text, progress=None):
    progress = 0 if progress is None else max(0, min(float(progress), 1))
    complete = " complete" if progress >= 1 else ""
    st.markdown(
        f"""
        <div class="kpi-card">
            ...
            <div class="progress-track"><div class="progress-fill{complete}" style="width:{progress*100:.0f}%"></div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    st.markdown('<div class="brand">NEWCASTLE OS</div><div class="brand-sub">Acquisition Intelligence</div>', unsafe_allow_html=True)
    st.markdown("---")
    page = st.radio("Workspace", ["Dashboard", "Comp Analyzer", "Hot Leads", "Transactions", "Reports", "Settings"], label_visibility="collapsed")
    st.markdown("---")
...
if page == "Dashboard":
    ...
    st.markdown('<div class="hero-title">Good evening, Marco.</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-copy">Track the activity that creates contracts—not vanity metrics.</div>', unsafe_allow_html=True)

    # Sample values now; these will be replaced by Bigin/live call data in the integration step.
    metrics = [
    ...
    with left:
        st.markdown("### Pipeline pulse")
        st.markdown("""
        <div class="panel">
            <div style="display:flex;justify-content:space-between;margin-bottom:14px"><span style="color:#8d98aa">New lead response</span><b>4m 18s</b></div>
            <div style="display:flex;justify-content:space-between;margin-bottom:14px"><span style="color:#8d98aa">Offers awaiting response</span><b>6</b></div>
            <div style="display:flex;justify-content:space-between;margin-bottom:14px"><span style="color:#8d98aa">Projected assignment revenue</span><b style="color:#57e7b1">$63,000</b></div>
            <div style="display:flex;justify-content:space-between"><span style="color:#8d98aa">Weekly contract pace</span><b>On target</b></div>
        </div>
        """, unsafe_allow_html=True)
    with right:
        ...
        st.markdown("""
        <div class="panel">
            <div class="hot-pill">HOT LEAD</div>
            <h4 style="margin:.7rem 0 .25rem">John Smith · Stockton</h4>
            <div style="color:#8d98aa;font-size:.84rem">Probate · 7-day timeline · Asking $240,000</div>
            <hr style="border-color:#252d3b;margin:16px 0">
            <div class="hot-pill">FOLLOW UP</div>
            <h4 style="margin:.7rem 0 .25rem">Mary Jones · Fresno</h4>
            <div style="color:#8d98aa;font-size:.84rem">Relocating · Offer sent yesterday</div>
        </div>
        """, unsafe_allow_html=True)

elif page == "Comp Analyzer":
    st.markdown('<div class="eyebrow">Property intelligence</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-title">Comp Analyzer</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-copy">Strict sold comps, fast underwriting, and a clear offer range.</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        photo_link = st.text_input("Property Photo Folder", placeholder="Paste Dropbox or Google Drive link")
    with col2:
    ...
    if analyze:
        ...
        st.markdown("### ARV and offer matrix")
        ...
        tiers = [("75% ARV", .75), ("70% ARV", .70), ("65% ARV", .65), ("60% ARV", .60)]
        for col, (label, pct) in zip(st.columns(4), tiers):
            offer = (arv * pct - repair_estimate) if arv else None
            col.metric(label, money(offer))
            col.caption(f"{label} minus repairs")

        st.markdown("### Sold comparable sales")
        st.caption("Sold only · same beds/baths · ±300 sq ft · within 0.50 miles · newest first")
        if not comps:
            st.warning("No comps matched the strict criteria.")
        else:
            rows = []
            for c in comps:
        ...
        if arv:
            st.markdown(f"""
            <div class="panel"><b>Acquisition guidance</b><br><br>
            Start with the 70% ARV tier unless buyer demand supports a more aggressive offer.<br><br>
            <span style="color:#8d98aa">Rules used: {comp_window}, same beds/baths, ±300 sq ft, within 0.50 miles, sold only.</span>
